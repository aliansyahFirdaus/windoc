...
function isTextLikeElement(element: IElement): boolean {
  return (
    !element.type || TEXTLIKE_ELEMENT_TYPE.includes(element.type as ElementType)
  );
}
...
function computeCatalog(
  ...
  const recursiveInsert = (
    ...
    const catalogItemLevel =
      titleOrderNumberMapping[subCatalogItem?.level as TitleLevel];
  ...
  for (let e = 0; e < titleElementList.length; e++) {
    ...
    const catalogItemLevel =
      titleOrderNumberMapping[catalogItem?.level as TitleLevel];
    const titleLevel = titleOrderNumberMapping[title.level as TitleLevel];
