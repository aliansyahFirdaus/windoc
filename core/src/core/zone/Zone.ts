...
export class Zone {
  ...
  public drawZoneIndicator() {
    ...
    const indicatorTop = isHeaderActive ? 0 : footer.getEffectiveTopY();
    for (let p = 0; p < pageList.length; p++) {
      ...
      const indicatorTopY = isHeaderActive ? startY : startY + indicatorHeight;
  ...
  private _toggleDropdown(anchor: HTMLElement, isHeaderActive: boolean) {
    ...
    if (isDisabled) {
      ...
      formatItem.textContent = isHeaderActive
        ? 'Header Format'
        : 'Footer Format';
