...
export class CommandAdapt {
  ...
  public title(payload: TitleLevel | null) {
    ...
    changeElementList.forEach(el => {
      if (payload) {
        el.level = payload
        el.titleId = titleId
        // isTextLikeElement sudah mencakup ZERO (no type = text-like)
        // sehingga empty heading line juga dapat size/bold yang benar
        if (isTextLikeElement(el)) {
          el.size = titleOptions[titleSizeMapping[payload]]
          el.bold = true
        } else if (el.type === ElementType.LABEL) {
          el.size = Math.max(titleOptions[titleSizeMapping[payload]] - 1, 8)
          el.bold = true
        }
      } else {
        if (el.titleId) {
    ...
    // Update defaultStyle agar karakter baru yang diketik di heading
    // langsung inherit ukuran dan bold heading tersebut
    if (payload) {
      this.range.setDefaultStyle({
      ...
      this.range.setDefaultStyle(null)
    }
  ...
  public listWithPreset(
    ...
    const isReadonly = this.draw.isReadonly()
    if (isReadonly) return
    this.draw.getListParticle().setListWithPreset(listType, listStyle, preset)
  }
  ...
  public addWatermark(payload: IWatermark) {
    ...
    options.watermark.rotation = payload.rotation ?? defaultWatermarkOption.rotation
    options.watermark.inFront = !!payload.inFront
    this.draw.render({
      isSetCursor: false,
      isSubmitHistory: false,
      isCompute: false
  ...
  public columnCount(payload: number) {
    this.draw.setColumnCount(payload)
  }
  ...
  public columnBreak() {
    ...
    this.insertElementList([
      { type: ElementType.COLUMN_BREAK, value: WRAP },
      { value: ZERO }
    ])
  }

  public columnGap(gap: number) {
    this.draw.setColumnGap(gap)
  }
}
