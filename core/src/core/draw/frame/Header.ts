...
export class Header {
  ...
  // Header area extends at least to the top margin line
  public getEffectiveBottomY(): number {
    const margins = this.draw.getMargins()
    const contentBottomY = this.getHeaderTop() + this.getHeight()
    return Math.max(contentBottomY, margins[0])
  }
