...
export class Draw {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (
        ...
        metrics.boundingBoxDescent =
          preElement?.metrics.boundingBoxDescent || 0;
      } else if (
          ...
          for (let t = 0; t < constrainedTrList.length; t++) {
            ...
            for (let d = 0; d < tr.tdList.length; d++) {
              ...
              while (idx < td.rowspan) {
                const curTr =
                  constrainedTrList[idx + t] || constrainedTrList[t];
                curTdMinHeight += curTr.minHeight!;
                curTdRealHeight += curTr.height!;
                idx++;
              }
      ...
      if (isWrap || i === elementList.length - 1) {
        ...
        if (
          ...
          if (spaceIndices.length > 0) {
            const gap = (availableWidth - curRow.width) / spaceIndices.length;
  ...
  private _applyRowSpacing() {
    ...
    const HEADING_SPACE: Partial<
      Record<TitleLevel, { above: number; below: number }>
    > = {
    ...
    if (pageMode === PageMode.CONTINUITY) {
      ...
      for (let i = 0; i < this.rowList.length; i++) {
        ...
        const rowTotalHeight =
          ...
          (row.spaceBelow || 0);
        if (
          rowTotalHeight + pageHeight > height ||
          isPageBreak ||
          isColumnBreak
        ) {
          if (
  ...
  private _drawPage(payload: IDrawPagePayload) {
    ...
    if (
      ...
      !this.options.watermark.inFront
    ) {
